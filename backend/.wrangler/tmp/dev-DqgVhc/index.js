...
async function signJwt(payload, env) {
  ...
  return await new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setExpirationTime(expires).sign(secret);
}
__name(signJwt, "signJwt");
...
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  if (!token)
    return c.json({ message: "Invalid token", code: 401 });
...
router.post("/login", async (c) => {
  ...
  const env = c.env;
  const db = env.DB;
  const row = await db.prepare("SELECT * FROM users WHERE username = ? AND password = ?").bind(username, password).first();
  if (!row) {
    return c.json({ message: "Invalid credentials" }, 400);
  }
  const role = await db.prepare("SELECT * FROM roles WHERE id = ?").bind(row.role_id).first();
  ...
  if (role?.value === "super_admin") {
    ...
    const roleMenus = await db.prepare(
      `SELECT m.permission 
         ...
         WHERE rm.role_id = ?`
    ).bind(row.role_id).all();
    const perms = (roleMenus.results || []).map((r) => r.permission).filter(Boolean).flatMap((p) => p.split(",")).filter(Boolean);
...
router.get("/profile", authMiddleware, async (c) => {
  ...
  const env = c.env;
  const db = env.DB;
  const userRow = await db.prepare("SELECT id, username, nickname, avatar FROM users WHERE id = ?").bind(user.id).first();
  if (!userRow) {
...
var permission = /* @__PURE__ */ __name((permRequired) => async (c, next) => {
  const user = c.get("user");
...
router2.get("/", permission("user:view"), async (c) => {
  const env = c.env;
  const db = env.DB;
  const rows = await db.prepare("SELECT id, username, nickname, avatar, role_id, status, created_at FROM users").all();
  return c.json(rows.results || []);
});
router2.post("/", permission("user:add"), async (c) => {
  const body = await c.req.json();
  const env = c.env;
  const db = env.DB;
  const res = await db.prepare("INSERT INTO users (username, password, nickname, avatar, role_id, created_at) VALUES (?, ?, ?, ?, ?, datetime('now'))").bind(body.username, body.password, body.nickname ?? "", body.avatar ?? "", body.role_id ?? null).run();
  return c.json({ id: res.meta.last_row_id });
});
router2.put("/:id", permission("user:edit"), async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  const env = c.env;
  const db = env.DB;
  await db.prepare("UPDATE users SET nickname = ?, avatar = COALESCE(?, avatar), password = COALESCE(?, password), role_id = COALESCE(?, role_id), status = COALESCE(?, status) WHERE id = ?").bind(body.nickname ?? "", body.avatar ?? null, body.password ?? null, body.role_id ?? null, body.status ?? null, id).run();
  return c.json({ message: "updated" });
});
router2.delete("/:id", permission("user:delete"), async (c) => {
  const id = c.req.param("id");
  const env = c.env;
  const db = env.DB;
...
router4.get("/authorized", authMiddleware, async (c) => {
  const env = c.env;
  const db = env.DB;
  const user = c.get("user");
  ...
  if (!isSuperAdmin) {
    const userRow = await db.prepare("SELECT role_id FROM users WHERE id = ?").bind(user.id).first();
    if (!userRow) {
      return c.json([]);
    }
    const roleMenus = await db.prepare("SELECT menu_id FROM role_menu WHERE role_id = ?").bind(userRow.role_id).all();
    authorizedMenuIds = (roleMenus.results || []).map((r) => String(r.menu_id));
...
var menus_default = router4;

// src/routes/r2.ts
var router5 = new Hono2();
router5.use("*", authMiddleware);
router5.get("/", permission("r2:view"), async (c) => {
  const env = c.env;
...
router5.post("/upload", permission("r2:add"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
  ...
  if (uploadId && partNumber && key) {
    ...
    try {
      ...
      const uploadedPart = await multipartUpload.uploadPart(partNum, arrayBuffer2);
      return c.json({
        uploadId,
        partNumber: partNum,
...
router5.post("/upload/init", permission("r2:add"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
  const body = await c.req.json();
  if (!body.key) {
...
router5.post("/upload/complete", permission("r2:add"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
  const body = await c.req.json();
...
router5.get("/preview/:key(*)", permission("r2:view"), async (c) => {
...
router5.get("/proxy/:key(*)", permission("r2:view"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
...
router5.post("/folder", permission("r2:add"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
  const body = await c.req.json();
  if (!body.path) {
...
router5.put("/move", permission("r2:edit"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
  const body = await c.req.json();
...
router5.get("/info/:key(*)", permission("r2:view"), async (c) => {
  const env = c.env;
  const r2 = env.R2_STORAGE;
  const keyParam = c.req.param("key");
  if (!keyParam) {
...
app.route("/roles", roles_default);
app.route("/menus", menus_default);
app.route("/r2", r2_default);
app.get("/", (c) => c.text("OK"));
var src_default = app;
